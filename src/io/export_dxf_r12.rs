...
mod tests {
    use super::*;
    use acadrust::entities::{Line, LwPolyline, LwVertex};
    ...
    fn exports_binary_ac1009_line_and_finite_extents() {
        ...
        .unwrap();
        let (bytes, report) = export_to_bytes(&doc).expect("export");
        assert!(bytes.starts_with(SENTINEL));
        assert_eq!(report.entity_count, 1);
        let reread = DxfReader::from_reader(Cursor::new(bytes)).unwrap().read().unwrap();
        assert_eq!(reread.version, acadrust::DxfVersion::Unknown);
        assert_eq!(reread.model_space_extents_min, Vector3::ZERO);
        assert_eq!(reread.model_space_extents_max, Vector3::new(20.0, 20.0, 0.0));
    }
    ...
    fn converts_bulged_lwpolyline_without_mutating_source() {
        ...
        let reread = DxfReader::from_reader(Cursor::new(bytes)).unwrap().read().unwrap();
        let EntityType::Polyline2D(converted) = reread.entities().next().unwrap() else { panic!("expected POLYLINE") };
    ...
    fn rejects_unsupported_entities() {
        let mut doc = CadDocument::new();
        doc.add_entity(EntityType::Text(acadrust::entities::Text::new()))
            .unwrap();
        let error = export_to_bytes(&doc).unwrap_err();
